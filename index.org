...
* emacs  - [[https://github.com/hy36370637/Dotemacs][emacs 설정 repository]]
** 한글맞춤법
*** hunspell 설치
    #+begin_src 
...
** 자동완성
   - Hippie Expand 
     + dabbrev와 유사하지만 더 강력하고 유연한 기능을 제공하는 자동 완성 기능
     + 기본적으로 M-/ 키에 바인딩. 입력 도중 단축키 눌러 입력 편의 도움
       #+begin_src emacs-lisp
  ...
  (defun try-expand-org-keyword (old)
    ...
    (if (null he-expand-list)
      ...
      t)))
       #+end_src   
     + abbrev
     + Emacs의 내장 기능. 사용자가 정의한 약어 → 자동 변환
     + 예를 들어, "mymail"를 입력하면 자동으로 "000@mail.com"로 변환
       #+begin_src emacs-lisp
(use-package abbrev
  :ensure nil				;built-in
  :config
